...
void solve() {
    ...
    int max = abs(1-i)+abs(1-j);
    int x1 = 1, y1 = 1, x2 = m, y2 = n;
    
    if((abs(1-i)+abs(n-j))>max) {x1 = 1; y1 = n; x2 = m; y2 = 1; max = (abs(1-i)+abs(n-j));}
    if((abs(m-i)+abs(1-j))>max) {x1 = m; y1 = 1; x2 = 1; y2 = n; max = (abs(m-i)+abs(1-j));}
    if((abs(m-i)+abs(n-j))>max) {x1 = m; y1 = n; x2 = 1; y2 = 1; max = (abs(m-i)+abs(n-j));}
