...
void solve() {
    ...
    string s; cin>>s;
    bool flag = true; //I didn't read the question, where for a single character the process was defined
    forin(i,1,s.size()) {
        if(((int)s[i]>=65) && ((int)s[i]<=90)) flag = true;
        else {flag = false; break;} //I didn't put the break condition in the beginning, keep track of it, if you are making a flag false, you need to remember to break the loop as well if needed
    }
    if(flag == true) {
        if((int)s[0]>=97&&(int)s[0]<=122) s[0] = char(s[0]-32);
        else if ((int)s[0]>=65&&(int)s[0]<=90) s[0] = char(s[0]+32);
        forin(i,1,s.size()) s[i] = char(s[i]+32);
    }
    cout<<s<<endl;
}
